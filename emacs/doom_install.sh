...
# This is one of the *standard* DOOMDIR locations expected by Doom. If
# XDG_CONFIG_HOME is not set, Doom should default to ~/.config, so
# everything should work fine even without symlinking this to ~/.doom.d
# (the other standard location for DOOMDIR, which doesn't depend on
...
# Ditto for this location vs. ~/.emacs.d, except this depends on fairly
# recent support for XDG_CONFIG_HOME in Emacs itself. According to
# Emacs's etc/NEWS.27 file, it looks like XDG_CONFIG_HOME will also
# fall back to ~/.config if not set -- let's hope so.
#
# Ideally, if you want to test a vanilla Emacs config, just create an
