...
(after! org-re-reveal
  (setq!
    org-re-reveal-head-preamble "<style>
/* This should at the very least apply to .csl-bib-body, but it's probably more readable to have text on all slides left-aligned. */
.reveal .slides {
  text-align: left;
}
/* Make slides scrollable, except for title slides or explicitly centered ones (setting a 100% height breaks the centering, and there's no purpose in centering content that overflows). */
.slides > section > section:not(:first-child):not(.center) {
  height: 100%%;
  overflow-y: auto;
}
/* Make it less likely that 4:3 aspect ratio high DPI images will vertically overflow. */
.reveal img {
  max-width: 85%%;
}
</style>
...
(setq dlukes/jupyter-max-image-width 600)
(defadvice! dlukes/jupyter-max-image-width (oldfun file width height)
  "Limit width via attr_org to play nicely with visual-fill-column."
  :around 'jupyter-org-image-link
  (message "max-width: %d" dlukes/jupyter-max-image-width)
  (let* ((image-width (car (image-size
                            (create-image (expand-file-name file))
                            'pixels)))
         (width (if (> image-width dlukes/jupyter-max-image-width)
                  dlukes/jupyter-max-image-width
                  nil)))
    (funcall oldfun file width height)))
